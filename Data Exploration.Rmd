...
Mittelwert_matrix = t(apply(MS_Table_norm, 1, function(row) {
  sapply(1:50, function(i) {
    ...
    werte = row[spalten_index]
    werte = werte[werte != 0]  # 0 entfernen
    if (length(werte) > 0) {
      mean(werte)
    } else {
      0  # Falls alle 3 Werte 0 sind
    }
  })
}))
...
for (i in 1:nrow(shared_peaks)) {
  ...
  # t-Test nur wenn beide Gruppen mindestens 2 Werte haben (zweiseitiger t-test)
...
hol_shifts <- function(protein, threshold_rel = 0.1) {
...
-   p-Wert muss < 0.05 sein damit signifikant wenn dann diese maxima stellen miteinander verglichen werden und der shift =0 ist dann kein shift
...
wenn das gegeben ist dann 100% ein Shiftendes Protein = RNA-Abhängigkeiten > NEIN nur wenn 

-   wenn Anzahl an Maxima sich verändert dann shift

wenn summe verlust + summe gewinn == 0 dann kein shift

p-werte sind aus richtung von kontrolle > wenn dann erst NA werte heißt die werte fehlen da
...
```{r}
berechne_shift_merkmale <- function(res) {
  # p-Werte
  p_signifikant <- any(res$p_Werte < 0.05, na.rm = TRUE)

  # Amplitudensummen
  amplitude_total <- res$Summierter_Gewinn + res$Summierter_Verlust

  # Anzahl Maxima
  diff_maxima <- res$Anzahl_RNase_Maxima - res$Anzahl_Kontroll_Maxima

  # Shiftrichtung über Maximadifferenz
  richtung <- "kein Shift"
  if (res$Shift_idx > 0) {
    richtung <- "right"
  } else if (res$Shift_idx < 0) {
    richtung <- "left"
  }

  # Fälle behandeln, wo Shift_idx = 0
  if (res$Shift_idx == 0) {
    if (diff_maxima != 0) {
      # Interpretationslogik bei gleicher Lage aber Peak-Gewinn/Verlust
      if (res$Summierter_Verlust > 0 && res$Summierter_Gewinn > 0) {
        richtung <- ifelse(which.max(c(res$Summierter_Gewinn, res$Summierter_Verlust)) == 1, "right", "left")
      } else if (res$Summierter_Gewinn > 0) {
        richtung <- "right"
      } else if (res$Summierter_Verlust > 0) {
        richtung <- "left"
      }
    } else {
      richtung <- "kein Shift"
  ...
  # Shift-Score-Kategorie
  kategorie <- "kein Shift"

  if (!is.na(amplitude_total) && (amplitude_total < -1 || amplitude_total > 1)) {
    if (p_signifikant) {
      kategorie <- paste("signifikanter", richtung, "Shift")
    } else {
      kategorie <- paste("nicht signifikanter", richtung, "Shift")
    }
  }

  # Rückgabe als Liste
  return(list(
    Score = amplitude_total,
    Richtung = richtung,
    Kategorie = kategorie,
    p_signifikant = p_signifikant
  ))
}
...
```{r}
protein_liste <- unique(c(Combined_Kontrolle$Protein, Combined_RNAse$Protein))

liste_von_dfs <- lapply(protein_liste, function(protein) {
  res <- hol_shifts(protein)
  shift_info <- berechne_shift_merkmale(res)
  ...
  data.frame(
    ...
    Summierter_Verlust = res$Summierter_Verlust,
    Summierter_Gewinn = res$Summierter_Gewinn,
    Amplitude_total = shift_info$Score,
    Abstaende_Maxima = paste(res$Abstände_Maxima, collapse = ";"),
    Verlust_Amplituden = paste(res$Verlust_Amplituden, collapse = ";"),
    Gewinn_Amplituden = paste(res$Gewinn_Amplituden, collapse = ";"),
    Fläche_Kontrolle = paste(res$Fläche_Kontrolle, collapse = ";"),
    Fläche_RNase = paste(res$Fläche_RNase, collapse = ";"),
    p_Werte = paste(res$p_Werte, collapse = ";"),
    Shift_Score = shift_info$Score,
    Richtung = shift_info$Richtung,
    Kategorie = shift_info$Kategorie
  )
})

df_alle_proteine <- do.call(rbind, liste_von_dfs)
...
klassifiziere_protein_flexibel <- function(row) {
  # p-Wert checken
  p_vals <- parse_numeric_vector(row["p_Werte"])
  signifikant <- any(p_vals < 0.05, na.rm = TRUE)
  if (!signifikant) return("nicht_signifikant")
  
  # Werte extrahieren
  shift_idx <- as.numeric(row["Shift_idx"])
  max_kontrolle <- as.numeric(row["Anzahl_Kontroll_Maxima"])
  max_rnase <- as.numeric(row["Anzahl_RNase_Maxima"])
  sum_verlust <- as.numeric(row["Summierter_Verlust"])
  sum_gewinn <- as.numeric(row["Summierter_Gewinn"])
  
  verlust_list <- parse_numeric_vector(row["Verlust_Amplituden"])
  gewinn_list <- parse_numeric_vector(row["Gewinn_Amplituden"])
  
  # Gesamtamplitudenänderung
  amplitude_total <- sum_verlust + sum_gewinn
  
  # ❗️Wenn Amplitudenänderung klein → kein Shift
  if (amplitude_total >= -1 & amplitude_total <= 1) return("kein_shift")
  ...
  # Reihenfolge: Verlust vor Gewinn → right, umgekehrt → left
  if (length(verlust_list) > 0 && length(gewinn_list) > 0) {
    verlust_index <- which.max(verlust_list)
    gewinn_index <- which.max(gewinn_list)

    if (verlust_index < gewinn_index) {
      richtung <- "right"
    } else if (gewinn_index < verlust_index) {
      richtung <- "left"
    }
    shift_score <- shift_score + 1
  }
  ...
  if (shift_score == 0) {
    return("kein_shift")
  } else if (shift_score == 1) {
    return(paste0("potenziell_", richtung, "_shift"))
  } else {
    return(paste0(richtung, "_shift"))
  }
}

df_ergebnisse$Kategorie <- apply(df_ergebnisse, 1, klassifiziere_protein_flexibel)
