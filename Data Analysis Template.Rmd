...
# All significant p-values - haben wir die überhaupt irgendwo benutzt
...
**das ist noch nicht so wie es soll und das auch ne falsche Funktion**
!! nochmal schauen!!
+ da gibt es noch gar nicht die tabelle 
```{#r}
library(dplyr)
library(ggplot2)
...
```{r}
ggplot(df_clust_no_outlier, aes(x = Shift_Category, y = pI, fill = Shift_Category)) +
  geom_boxplot() +
...
ggplot(df_clust, aes(x = Shift_Category, y = pI)) +
...
# RBP-Status zuordnen
df_ergebnisse_phys <- df_ergebnisse_phys %>%
  mutate(RBP_Gruppe = ifelse(Shift_Category == "kein_shift", "non-RBP", "RBP"))
