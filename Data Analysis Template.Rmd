...
Normalization_plot = ggplot(df_all, aes(x = Fraction, y = Value_scaled, color = Condition, group = interaction(Condition, Replicate))) +
  ...
  labs(
    title = "Mean protein intensities per fraction before and after normalization",
    x = "Fraction",
    y = "Mean Intensity (%)",
    color = "Condition",
    linetype = "Replicate",
...
#### Protein distribution of the two treatments for an example protein to get an overview of the differences - als Überschrift bisschen zu lang
...
# Eventuell könnten wir dann nochmal das anzeigen mit den sachen dann schon angezeigt also das das da rechts steht in kombination was wir uns halt angeschaut haben

#### Create a dataframe with all these criteria for each protein - !!das verschiebt sich dann automatisch!!
...
# Bar plot: show total number of proteins of all shift categories and classify into RBP and non-RBP **das ist noch nicht so wie es soll und das auch ne falsche Funktion** !! nochmal schauen!! + da gibt es noch gar nicht die tabelle
...
ggplot(df_plot, aes(x = Mass_kDa, y = pI, color = predicted_score)) +
  ...
  labs(title = "Shift_Score based on mass and pI",
       x = "Mass (kDa)", y = "Isoelecktric point (pI)",
       color = "Score") +
  theme_poster
```
...
# Distribution of the shift categories differs significantly between the clusters
...
```{r}
library(ggplot2)
ggplot(df_clust, aes(x = Shift_Category, y = log10(Mass_kDa))) +
...
# Summen berechnen pro RBP_Gruppe + Shift_Category
df_summary <- df_ergebnisse_phys %>%
  group_by(RBP_Gruppe, Shift_Category) %>%
  summarise(n = n(), .groups = "drop") %>%
  mutate(
