...
<style>
  ...
  /* Überschriften linksbündig halten */
  h1, h2, h3, h4 {
    text-align: left;
  }
  /* Gesamtes Inhaltsverzeichnis leicht einrücken */
  #TOC ul {
    padding-left: 1.2em;
  }

  /* Listenelemente normal einrücken, kein hängender Einzug */
  #TOC ul li {
    text-indent: 0;
    white-space: normal; /* Zeilenumbruch erlauben */
  }

  /* Für Unterlisten (z.B. tiefere Ebenen) noch etwas mehr Einzug */
  #TOC ul ul {
    margin-left: 1em;
  }
</style>
...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, fig.width = 10, fig.height = 6, dpi = 600,
  fig.align = "center") 
```
...
```
![](Postervisuals/data_cleaning_workflow.png)
...
![](Postervisuals/protein_distribution.png)
...
##### Authors: Mirjam Biollaz, Jette Klempt-Gießing, Alicia Weeber
To assess whether the clusters formed from the normalized data align with our shift-categories, we visualized the proteins in the PCA colored by their respective shift category.
```{r}
# Your PCA object
pca = prcomp(MS_Table_norm, center = TRUE, scale. = TRUE)
...
color_cluster = shift_colors[as.character(shift_kategorie)]
...
legend("center", legend=names(shift_colors),
       col=shift_colors, pch=19, title="Shift Category", cex=0.9)
```
It can be seen that the shift categories do not reflect the cluster analysis, and therefore we cannot draw any conclusions about the previously determined clusters. As a result, this analysis does not hold any further relevance for the next steps. 
...
")
svg = export_svg(overview_linear)
rsvg_png(charToRaw(export_svg(overview_linear)), "Postervisuals/overview_linear.png", width = 3000)
```

<div style="text-align: center;">
  <img src="Postervisuals/overview_linear.png" width="300">
</div>
...
digraph flowchart {
  ...
  # Defining knots
  pi [label = 'pI', shape = box, style = filled, fillcolor = '#7396AD']
  Zinc_finger [label = 'Zinc_finger', shape = box, style = filled, fillcolor = '#728083']
  RBP_classification [label = 'RBP_classification', shape = box, style = filled, fillcolor = '#485571']
...
svg = export_svg(overview_logistic)
rsvg_png(charToRaw(export_svg(overview_logistic)), "Postervisuals/overview_logistic.png", width = 3000)
```
![](Postervisuals/overview_logistic.png){width=300}
