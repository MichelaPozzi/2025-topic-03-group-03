...
Ziel dieser Analyse ist es, Proteom-Daten aus HeLa-Zellen zu untersuchen, die über Fraktionierungsexperimente mit und ohne RNase-Behandlung generiert wurden. Die Analyse beinhaltet Qualitätskontrolle, Normalisierung und Visualisierung der Signalverteilung über Fraktionen hinweg, um z. B. mögliche Veränderungen in der Proteinverteilung nach RNase-Behandlung sichtbar zu machen.
...
```{r}
all(sapply(MS_Table, is.numeric)) # TRUE = alle Spalten sind numerisch
```
...
```{r Entfernen von Proteinen ohne Daten, author: Alle}
...
Das schönste was wir bisher haben aber mein Probkem ich verstehe nich was hier gemacht wird

Veräbderubgen: **Achsenbeschriftung muss weiter weg + vor muss links**
...
```{r Scatterplost für Korrelationen, author = "Hasset"}
library(stringr)
library(dplyr)
library(ggplot2)
library(tidyr)
