...
model = lm(Kategorie ~ pI + TM_Count , data = df_ergebnisse_phys)
summary(model)
...
df_ergebnisse_phys$RBP_Status = factor(df_ergebnisse_phys$RBP_Status, levels = c("NO_RBP", "RBP"))

# Logistische Regression für Masse, Länge, pI
model_logit = glm(RBP_Status ~ Mass_kDa + Length_AA + pI,
                   data = df_ergebnisse_phys,
                   family = "binomial")
summary(model_logit)

df_ergebnisse_phys$fitted_Status = predict(model_logit, newdata = df_ergebnisse_phys)

```

```{r}
# Logistische Regression für Zinc_Count
model_phys <- glm(RBP_Status ~ Zinc_Count + Coil_Count,
                   data = df_ergebnisse_phys,
                   family = "binomial")
...
```{r}
conf_matrix = table(df_ergebnisse_phys$RBP_Status, df_ergebnisse_phys$fittet_Status)
colnames(conf_matrix) <- c("Vorhergesagt_NO_RBP", "Vorhergesagt_RBP")
rownames(conf_matrix) <- c("Tatsächlich_NO_RBP", "Tatsächlich_RBP")
conf_matrix
