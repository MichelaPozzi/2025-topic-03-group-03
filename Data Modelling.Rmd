...
model = lm(Kategorie ~ pI, data = df_ergebnisse_phys)
summary(model)
...
```{r}
df_ergebnisse_phys$RBP_nach_Score <- ifelse(df_ergebnisse_phys$Kategorie == "kein_shift", "NO_RBP", "RBP")
...
```{r}
# Zielvariable als Faktor (0/1-artig, wichtig!)
df_ergebnisse_phys$RBP_nach_Score = factor(df_ergebnisse_phys$RBP_nach_Score, levels = c("NO_RBP", "RBP"))
...
complete_idx <- complete.cases(df_ergebnisse_phys[, c("Zinc_Count", "pI")])

# Confusion Matrix mit passenden Zeilen aus beiden Dataframes
conf_matrix <- table(
  True = df_ergebnisse_phys$RBP_nach_Score[complete_idx],
  Predicted = df_pred$fitted_Status
)
...
```{r}
# 1) Indizes für vollständige Werte (keine NA in den Prädiktoren)
complete_idx <- complete.cases(df_ergebnisse_phys[, c("Zinc_Count", "pI")])

# 2) Datenframe mit vollständigen Fällen (RBP_nach_Score und Prädiktoren)
df_complete <- df_ergebnisse_phys[complete_idx, ]

# 3) Vorhersagen für diese vollständigen Fälle
df_complete$fitted_Status <- predict(model_logit, newdata = df_complete, type = "response")

# 4) Binäre Klassifikation nach Schwellenwert 0.5
...
# Optional Spalten und Zeilen beschriften
colnames(conf_matrix) <- c("Vom Modell vorhergesagt: NO_RBP", "Vom Modell vorhergesagt: RBP")
rownames(conf_matrix) <- c("Klassifikation: NO_RBP", "Klassifikation: RBP")
...
```{r}
ggplot(df_ergebnisse_phys, aes(x = Zinc_Count, y = pI)) +
  stat_density_2d(aes(fill = ..level..), geom = "polygon") +
  facet_wrap(~RBP_nach_Score) +
  labs(x = "Zink-Finger", y = "pI", title = "2D-Dichte von Zinc & pI nach RBP-Status") +
  theme_minimal()
```
