...
df_uniprot_counts <- df_uniprot %>%
  mutate(
    ...
    Motif_Count = ifelse(is.na(Motif)         | Motif == "",         0, lengths(Motif_List))
  ) %>%
  select(Entry, `Entry Name`, Zinc_Count, Coil_Count, Motif_Count)
...
df_uniprot_counts1 <- df_uniprot %>%
  ...
  select(Entry, `Entry Name`, Zinc_Count, Coil_Count)
...
Aussage über Masse,Länge, pI über shift_score
```{r}
# physikalische Eigenschaften der Proteine, die übereinstimmen (RBP_Table), in df ergebnisse hinzufügen für die Regression
phys_eigenschaften = combined_table[, c("Entry_Name", "Mass_kDa", "Length_AA", "pI", "Listing_Count","Zinc_Count", "Coil_Count")]
...
model = lm(Shift_Score ~ pI + Zinc_Count , data = df_ergebnisse_phys)
summary(model)
