...
phys_eigenschaften = combined_table[, c("Entry_Name", "Mass_kDa", "Length_AA", "pI", "Kategorie")]
...
model = lm(Shift_Score ~ Mass_kDa + Length_AA + pI, data = df_ergebnisse_phys)
summary(model)
...
set.seed(42)
kmeans_result <- kmeans(scaled_data_no_outlier, centers = 6)
...
```{r}
# RBP Zuordnung basisierend auf unseren scores
df_ergebnisse_phys$RBP_nach_Score <- ifelse(df_ergebnisse_phys$Shift_Kategorie == "kein_shift", "non-RBP", "RBP")
df_ergebnisse_phys$RBP_nach_Score <- factor(df_ergebnisse_phys$rbp_status, levels = c("non-RBP", "RBP"))
```
