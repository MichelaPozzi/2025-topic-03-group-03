...
print(Boxplot_Korrelationswerte)
```
...
```
```{r}
p_dot <- ggplot(kor_long, aes(x = Paar, y = Korrelation, color = Paar)) +
  geom_jitter(width = 0.2, alpha = 0.7) +
  theme_minimal() +
  labs(title = "Einzelne Korrelationswerte (Jitter-Plot)",
       x = "Paar von Replikaten",
       y = "Korrelationskoeffizient") +
  theme(legend.position = "none")
...
```{r}
library(ggplot2)
library(viridis)  # für viridis Farbskala, falls installiert

p_heatmap_besser <- ggplot(kor_long, aes(x = Paar, y = Fraktion, fill = Korrelation)) +
  geom_tile() +
  #scale_fill_gradient(low = "white", high = "red", limits = c(0.93, 1), oob = scales::squish) +
  # Oder mit viridis:
  scale_fill_viridis_c(limits = c(0.94, 1), option = "D", oob = scales::squish) +
  theme_minimal() +
  labs(title = "Heatmap der Korrelationen pro Fraktion (angepasste Farbskala)",
       x = "Paar von Replikaten",
       y = "Fraktion") +
  theme(axis.text.y = element_text(size = 6))

print(p_heatmap_besser)
...
p_beeswarm <- ggplot(kor_long, aes(x = Paar, y = Korrelation, color = Paar)) +
  geom_beeswarm(cex = 2, alpha = 0.7) +
  theme_minimal() +
  labs(title = "Beeswarm-Plot der Korrelationen",
       x = "Paar von Replikaten",
       y = "Korrelationskoeffizient") +
  theme(legend.position = "none")
